...
contract Pool is IPool, ERC20 {
    ...
    int128 public constant k = int128(uint128(uint256((1 << 64)) / 126144000)); // 1 / Seconds in 4 years, in 64.64
    int128 public constant g1 = int128(uint128(uint256((950 << 64)) / 1000)); // To be used when selling Dai to the pool. All constants are `ufixed`, to divide them they must be converted to uint256
    int128 public constant g2 = int128(uint128(uint256((1000 << 64)) / 950)); // To be used when selling fyDai to the pool. All constants are `ufixed`, to divide them they must be converted to uint256
    ...
    function mint(
        ...
        uint256 tokensMinted = (supply * daiOffered) / daiReserves;
        uint256 fyDaiRequired = (fyDaiReserves * tokensMinted) / supply;

        require(daiReserves + daiOffered <= type(uint128).max); // daiReserves can't go over type(uint128).max
        require(supply + fyDaiReserves + fyDaiRequired <= type(uint128).max); // fyDaiReserves can't go over type(uint128).max
    ...
    function burn(
        ...
        uint256 fyDaiReturned;
        {
            ...
            uint256 fyDaiReserves = fyDai.balanceOf(address(this));
            daiReturned = (tokensBurned * daiReserves) / supply;
            fyDaiReturned = (tokensBurned * fyDaiReserves) / supply;
        }
