...
contract SoloMarginMock is SoloMarginLike {
    ...
    function operate(
        ...
        require(
            withdraw.amount.ref == DYDXDataTypes.AssetReference.Delta,
            "SoloMarginMock: incorrect withdraw reference"
        );

        require(
            withdraw.actionType == DYDXDataTypes.ActionType.Withdraw,
            "SoloMarginMock: incorrect withdraw action type"
        );

        /* call */
        DYDXDataTypes.ActionArgs memory call = actions[1];
        require(
            call.actionType == DYDXDataTypes.ActionType.Call,
            "SoloMarginMock: incorrect call action type"
        );

        /* deposit */
        DYDXDataTypes.ActionArgs memory deposit = actions[2];
        require(
            withdraw.primaryMarketId == deposit.primaryMarketId,
            "SoloMarginMock: marketId mismatch"
        );
        ...
        require(
            deposit.amount.ref == DYDXDataTypes.AssetReference.Delta,
            "SoloMarginMock: incorrect deposit reference"
        );

        require(
            deposit.actionType == DYDXDataTypes.ActionType.Deposit,
            "SoloMarginMock: incorrect deposit action type"
        );
        ...
        DYDXFlashBorrowerLike(msg.sender).callFunction(
            msg.sender,
            DYDXDataTypes.AccountInfo({owner: accounts[0].owner, number: accounts[0].number}),
            call.data
        );
