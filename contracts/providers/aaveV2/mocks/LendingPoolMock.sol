...
contract LendingPoolMock {
  ...
  function flashLoan(
    ...
    IAToken(aTokenAddress).transferUnderlyingTo(receiverAddress, amounts[0]);

    require(
      IAaveV2FlashBorrower(receiverAddress).executeOperation(assets, amounts, premiums, msg.sender, params),
  ...
  function addReserve(address aToken)
    external
  {
    address asset = IAToken(aToken).underlying();
