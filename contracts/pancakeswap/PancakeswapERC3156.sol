    ...
    IPancakeCallee
{
    ...
    constructor(address _factory, address _feeTo) public {
        ...
        require(
            ...
            "PancakeswapERC3156: feeTo address is zero address!"
        );
        factory = _factory;
        FEETO = _feeTo;
    }
    ...
    function flashFee(address _token, uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 pancakeFee = _pancakeFee(_token, _amount);
        uint256 additionalFee = _additionalFee(_amount);
        uint256 totalFee = pancakeFee.add(additionalFee);
        return totalFee;
    }

    function _pancakeFee(address _token, uint256 _amount)
        internal
        view
        returns (uint256)
    {
    ...
    function pancakeCall(
        ...
        uint256 pancakeFee = _pancakeFee(token, amount);
        IERC20(token).transfer(msg.sender, amount.add(pancakeFee));
    }
}
