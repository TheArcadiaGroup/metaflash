...
describe('UniswapV3', () => {
  ...
  it("add/removePairs", async function () {
    //add
    await expect(lender.connect(user).addPairs([ONE_ADDRESS], [ONE_ADDRESS], [ONE_ADDRESS])).to.revertedWith('UniswapV3FlashLender: Not operator');
    await expect(lender.addPairs([ONE_ADDRESS], [ONE_ADDRESS], [ONE_ADDRESS, ONE_ADDRESS])).to.revertedWith('UniswapV3FlashLender: mismatch length of token0, token1, pair');
    await expect(lender.addPairs([ZERO_ADDRESS], [ONE_ADDRESS], [ONE_ADDRESS])).to.revertedWith('UniswapV3FlashLender: _tokens0 is address(0)');
    await expect(lender.addPairs([ONE_ADDRESS], [ZERO_ADDRESS], [ONE_ADDRESS])).to.revertedWith('UniswapV3FlashLender: _tokens1 is address(0)');
    await expect(lender.addPairs([ONE_ADDRESS], [ONE_ADDRESS], [ZERO_ADDRESS])).to.revertedWith('UniswapV3FlashLender: _pairs is address(0)');
    ...
    //remove
    await expect(lender.connect(user).removePairs([ONE_ADDRESS])).to.revertedWith('UniswapV3FlashLender: Not operator');
