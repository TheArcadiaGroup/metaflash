...
describe('Pancakeswap', () => {
  ...
  it('flash fee', async function () {
    expect(await lender.flashFee(wethDaiPairAddress, weth.address, reserves)).to.equal((reserves.mul(25).div(9975).add(1)));
    expect(await lender.flashFee(wethUsdcPairAddress, usdc.address, reserves)).to.equal((reserves.mul(25).div(9975).add(1)));
  });
  ...
  it('invalid case - flashLoan', async () => {
    ...
    for (let i = 0; i < maxloans.length; i++) {
      ...
      await expect(borrower.connect(user).flashBorrow(pairs[i], lender.address, weth.address, tempBal, { gasLimit: 30000000 })).to.revertedWith("ERC20: transfer amount exceeds balance");
      count++;
      if (count == 2) {
        break;
      }
